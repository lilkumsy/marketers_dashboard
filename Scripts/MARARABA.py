...
cursor.execute("select a.AccountDesc as accountName,a.AmountGranted  as amountgranted from VwCreditMaster a,VwAccMaster b where a.AccountNo=b.AccountNo and a.CreditTypeDesc='Mortgage Loan' and a.PostingDateAdded between '2023-01-01' and '2023-12-01' and RelationshipOfficerName in('OPEGBEMI, FUNMILAYO  ')")
bar_data = cursor.fetchall()

# Execute the query to fetch data for the pie chart-pbt
query ="select sum(a.AmountGranted)   from VwCreditMaster a,VwAccMaster b where a.AccountNo=b.AccountNo and a.CreditTypeDesc='Mortgage Loan' and a.PostingDateAdded between '2023-01-01' and '2023-12-31' and RelationshipOfficerName in('OPEGBEMI, FUNMILAYO ')"
cursor = cnxn.cursor()
cursor.execute(query)
progress = cursor.fetchone()[0]
# update the `pie_data` list with the retrieved progress   
pbt_data = [('SET TARGET',   113373000.00), ('OUTSTANDING TARGET', progress)]
...
progress = cursor.fetchone()[0]
# update the `pie_data` list with the retrieved progress   
soh_data = [('SET TARGET',   37109583.55), ('OUTSTANDING TARGET', progress)]
# Specify colors for pie chart slices
colors = ['#00203FFF','#ADEFD1FF']  # green for SET TARGET, red for ACHIEVED TARGET
...
progress = cursor.fetchone()[0]
# update the `pie_data` list with the retrieved progress   
pie_data = [('SET TARGET',  1326000000.00 ), ('OUTSTANDING TARGET', progress)]
# Specify colors for pie chart slices
colors = ['#00203FFF','#ADEFD1FF']  # green for SET TARGET, red for ACHIEVED TARGET

# Execute the query to fetch data for the pie chart2-target(deposits)
query2 ="select sum(a.BalAcctCurr) from VwTransHist a,VwAccMaster b where a.AccountNo=b.AccountNo and TransCode in('103','1031') and a.PostingDateAdded between '2023-01-01' and '2023-12-31' and  RelationshipOfficerName in('OPEGBEMI, FUNMILAYO ')"
cursor2 = cnxn.cursor()
cursor2.execute(query2)
progress2 = cursor2.fetchone()[0]
# update the `pie_data` list with the retrieved progress   
pie_data2 = [('SET TARGET',  939250000.00), ('OUTSTANDING TARGET', progress2)]
# Specify colors for pie chart slices
colors = ['#00203FFF','#ADEFD1FF']  # green for SET TARGET, red for ACHIEVED TARGET

# Execute the query to fetch data for the scatter diagram-NPL
cursor.execute("select AccountDesc as accdesc,BookBal as bkbal from VwAccMaster  where CreditClassDesc !='performing' AND RelationshipOfficerName in('OPEGBEMI, FUNMILAYO ') and PostingDateAdded <='2023-12-31' order by PostingDateAdded asc")
...
sidebar = html.Div(
    [
        html.H6("MARARABA BRANCH", className="display-6"),
        html.Hr(),
    ...
    figure={
        ...
        'layout': {
            ...
            'margin': {
                ...
                'r': 10,
                't': 40,
                'b': 80  # Increase the bottom margin to accommodate x-axis labels
            },
            'xaxis': {
                'automargin': True  # Enable automatic margin adjustment for x-axis labels
            },}})
        return html.Div(
            [
                html.H3("NPL"),
...
if __name__ == "__main__":
     app.run_server(debug='true',host='10.1.0.119', port=9004)
